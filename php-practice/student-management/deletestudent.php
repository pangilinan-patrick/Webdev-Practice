...
<body>
  ...
  <main>
    ...
    <form class="form__layout" method="post">
      <section>
        <header>
          <h2 class="breadcrumbs">Student / Delete Student</h2>
        </header>
        <table class="edit_Table">
          <tr>
            <td><label class="input-label" for="input_student_number">Student No:</label></td>
            <td><input class="input-field" type="number" value="<?= $u_student_number; ?>" disabled></td>
          </tr>
          <tr>
            <td><label class="input-label" for="input_last_name">Last Name:</label></td>
            <td><input class="input-field" type="text" value="<?= $u_last_name; ?>" disabled></td>
          </tr>
          <tr>
            <td><label class="input-label" for="input_first_name">First Name:</label></td>
            <td><input class="input-field" type="text" value="<?= $u_first_name; ?>" disabled></td>
          </tr>
          <tr>
            <td><label class="input-label" for="input_middle_name">Middle Name:</label></td>
            <td><input class="input-field" type="text" value="<?= $u_middle_name; ?>" disabled></td>
          </tr>
          <tr>
            <td><label class="input-label" for="input_gender">Gender:</label></td>
            <td><input class="input-field" type="text" value="<?= $u_gender; ?>" disabled></td>
          </tr>
        </table>
        <?php
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
          $sql = "DELETE FROM tblstudent WHERE fIdstudentno = $ID";

          if ($_POST["button"] === "Delete Student") {
            if (mysqli_query($con, $sql)) {
              mysqli_query($con, "SET @num := 0");
              mysqli_query($con, "UPDATE tblstudent SET fIdindex = @num := (@num+1)");
              mysqli_query($con, "ALTER TABLE tblstudent AUTO_INCREMENT = 1");
              echo "<div class='alert alert-success'>Record deleted! Going back to menu...</div>";
      ...
      <footer class="edit_footer">
        <input class="form__delete form__btn" type="submit" name="button" value="Delete Student">
